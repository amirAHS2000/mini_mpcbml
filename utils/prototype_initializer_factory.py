...
def initialize_mpcbml(
    ...
    **kwargs
    ...
    """
    Convenience wrapper for prototype initialization.
    
    Backward compatible with original initialize_mpcbml function.
    
    Args:
        model: Model for feature extraction
        train_loader: Training data loader
        criterion: Loss criterion (used to get config)
        device: Device (cuda/cpu)
        init_method: Initialization method ('random', 'kmeans', 'mean')
        **kwargs: Extra arguments
    
    Returns:
        Tuple of (prototypes, cluster_sizes)
    """
    factory = PrototypeInitializerFactory()
    
    # Get parameters from criterion if available
    n_classes = criterion.n_classes
    prototype_per_class = criterion.n_prototypes_per_class
    embed_dim = criterion.embed_dim
